function ret = pcaprobeval(vox, meanVox, sigmas, L, meanStdVols, mask, name, lambda)
% evaluate probability under pca model and with normal (gaussian) model of projection
    ...
    if nargin == 8
        [ret.logp, ret.scores] = pcaprob(meanVox, sigmas, vox, L, lambda);
    else
        [ret.logp, ret.scores] = pcaprob(meanVox, sigmas, vox, L);
    end
    
    % reconstruct the volume from the pca projection
    ret.recon = within([0, 1], pcarecon(L, ret.scores, meanVox));
